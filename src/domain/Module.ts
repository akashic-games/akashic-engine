 ...
 */
export function _require(game: Game, path: string, currentModule?: ModuleLike): any {
	// Node.js の require の挙動については http://nodejs.jp/nodejs.org_ja/api/modules.html も参照。

	var targetScriptAsset: AssetLike;
	...
	if (targetScriptAsset) {
		...
		if (targetScriptAsset.type === "script") {
			var context = new ScriptAssetContext(game, targetScriptAsset as ScriptAssetLike);
			...
			if (targetScriptAsset && PathUtil.resolveExtname(path) === ".json") {
				...
				var cache = (game._scriptCaches[resolvedPath] = new RequireCachedValue(
 ...
 */
export function _findAssetByPathAsFile(resolvedPath: string, liveAssetPathTable: { [key: string]: AssetLike }): AssetLike | undefined {
 ...
 * @param liveAssetPathTable パス文字列のプロパティに対応するアセットを格納したオブジェクト
 */
export function _findAssetByPathAsDirectory(resolvedPath: string, liveAssetPathTable: { [key: string]: AssetLike }): AssetLike | undefined {
	var path: string;
