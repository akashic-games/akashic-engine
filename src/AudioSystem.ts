...
export class MusicAudioSystem extends AudioSystem {
	/**
	 * 音声の同時再生可能数。
	 * 基本的には 1 以上 2 以下 (1 <= n <= 2) とし、コンテンツの実行開始タイミングもしくは音声を鳴らす前などで一度のみ値を代入することが望ましい。
	 * 本値は実験的なものである。以下の制限事項に留意して利用すること。
	 *
	 * * このインタフェースは将来的に変更される可能性がある
	 * * 本オーディオシステムに属するオーディオプレイヤーを再生中に本値を変更した場合の挙動は不定である
	 * * 同時再生数が本値を超えた場合の挙動は不定である
	 * * 環境や端末によって実際に再生できる数と異なることがある
	 */
